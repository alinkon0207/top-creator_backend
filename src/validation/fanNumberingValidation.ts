...
const changeFanNumberingSchema = yup.object({
    ...
    numbers: yup.number().min(2, 'Minimum numbers is 2').max(9, 'Maximum numbers is 9'),
    active: yup.boolean()
});

export async function validateChangeFanNumbering(input: ChangePromotionReactivatorInput) {
    try {
