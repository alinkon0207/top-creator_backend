...
import { ChangeMassMessagingInput, MassMessagingCreate } from '../generated/graphql';
...
const createMassMessagingSchema = yup.object({
    ...
    startDate: yup.date(),
    endDate: yup.date(),
    excludeFans: yup.number().min(0, 'Exclude Fans must be a non-negative number'),
...
export async function validateChangeMassMessaging(input: ChangeMassMessagingInput) {
