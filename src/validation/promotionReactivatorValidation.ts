...
const changePpvMessageSchema = yup.object({
    ...
    period: yup.number().min(1, 'Minimum period is 1').max(30, 'Maximum period is 30'),
    active: yup.boolean()
});

export async function validateChangePromotionReactivator(input: ChangePromotionReactivatorInput) {
    try {
