...
const welcomeSettingsMutationResolver = {
    Mutation: {
        async changeWelcomeSettings(_: never, { input }: { input: ChangeWelcomeSettingsInput }, context: any) {
           try {
               const token = authService.checkToken(context.token);

               const welcomeSettings = await welcomeSettingsService.changeWelcomeSettings(token, input);
